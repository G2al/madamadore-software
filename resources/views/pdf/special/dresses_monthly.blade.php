...
<head>
    <meta charset="UTF-8">
    <title>Abiti speciali da consegnare – {{ ucfirst($monthLabel) }}</title>
    <style>
        * {
...
<body>
    ...
    <h1>Abiti speciali da consegnare</h1>
    <h2>{{ ucfirst($monthLabel) }}</h2>

    @if($dresses->isEmpty())
        <p class="text-center" style="margin-top: 30px;">
            Nessun abito speciale con data di consegna in questo mese.
        </p>
    @else
        <table>
            <thead>
            ...
            <tbody>
                @foreach($dresses as $dress)
                    ...
                    <tr>
                        <td>
                            <strong>{{ $dress->customer_name ?? '-' }}</strong>
                        </td>
