...
namespace App\Filament\Resources\CompanyAdjustmentResource\Concerns;
...
trait HasCompanyAdjustmentTableDefinition
{
    ...
    private static function getStatusColumn(): Tables\Columns\TextColumn
    {
        return Tables\Columns\TextColumn::make('status')
            ->label('Stato')
            ->badge()
            ->formatStateUsing(fn(?string $state) => \App\Models\CompanyAdjustment::getStatusLabels()[$state] ?? '-')
            ->color(fn(?string $state) => \App\Models\CompanyAdjustment::getStatusColors()[$state] ?? 'gray')
            ->sortable();
    }
    ...
    private static function getTableFilters(): array
    {
        return [
            ...
            Tables\Filters\SelectFilter::make('status')
                ->label('Stato')
                ->options(\App\Models\CompanyAdjustment::getStatusLabels()),
     ...
     */
    private static function handleCompleteWork(CompanyAdjustment $record): void
    {
     ...
     */
    private static function handleToggleRitirato(CompanyAdjustment $record): void
    {
     ...
     */
    private static function handleDownloadReceipt(CompanyAdjustment $record)
    {
        ...
        return response()->streamDownload(
            fn() => print($pdf->output()),
            "ricevuta-aggiusto-aziendale-{$record->id}.pdf"
        );
    }
