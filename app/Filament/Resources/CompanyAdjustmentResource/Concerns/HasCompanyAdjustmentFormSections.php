...
trait HasCompanyAdjustmentFormSections
{
    /**
                ...
                Forms\Components\DatePicker::make('delivery_date')
                    ...
                    ->afterStateUpdated(function ($state, Set $set, Get $get, $livewire) {
                        if ($state) {
                            ...
                            $normalAdjustments = \App\Models\Adjustment::where('delivery_date', $state)->count();
                            $companyAdjustments = \App\Models\CompanyAdjustment::where('delivery_date', $state)
        ...
        return Forms\Components\Section::make('Pagamento')
            ->schema([
                Forms\Components\TextInput::make('client_price')
                    ...
                    ->rules(['gte:0'])
                    ->live()
                    ->afterStateUpdated(fn (Set $set, Get $get) => CompanyAdjustmentResource::updateCalculations($set, $get)),
        ...
        return Forms\Components\Section::make('Lista della spesa')
            ...
            ->schema([
                Forms\Components\Repeater::make('expenses')
                    ->label('Articoli')
                    ->relationship() // usa la relazione hasMany(CompanyAdjustmentExpense)
                    ->schema([
                        Forms\Components\TextInput::make('name')
                            ->label('Nome articolo')
